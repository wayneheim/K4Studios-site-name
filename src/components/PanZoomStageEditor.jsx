...
const SCALE_FACTOR = 1; // Preview renders at 100% actual pixel size for all aspect ratios
...
export default function PanZoomStageEditor({ slide, onSave, onCancel, resolution }) {
  ...
  // Calculate which stages are actually used for intro/outro based on metadata
  const usedStages = useMemo(() => {
    const isIntro = slide?.type === "intro" || slide?.id === "__INTRO__";
    const isOutro = slide?.type === "outro" || slide?.id === "__OUTRO__";
    
    if (!isIntro && !isOutro) return new Set(["start", "mid", "end"]);

    // Get intro/outro meta from slide
    const meta = slide?.introMeta || slide?.outroMeta || {};
    if (!meta.showIntroText && !meta.showShowTitle && !meta.showTagline) {
      // Old intro format, assume all stages are used
      return new Set(["start", "mid", "end"]);
    }

    const used = new Set();
    if (meta.showIntroText) used.add(["start", "mid", "end"][meta.introTextStage - 1] || "start");
    if (meta.showShowTitle) used.add(["start", "mid", "end"][meta.showTitleStage - 1] || "mid");
    if (meta.showTagline) used.add(["start", "mid", "end"][meta.taglineStage - 1] || "end");
    
    return used.size > 0 ? used : new Set(["start", "mid", "end"]);
  }, [slide]);
  ...
  const nextStage = () => {
    const stageOrder = ["start", "mid", "end"];
    const currentIdx = stageOrder.indexOf(stage);
    
    // Find next used stage
    let nextIdx = currentIdx + 1;
    while (nextIdx < stageOrder.length && !usedStages.has(stageOrder[nextIdx])) {
      nextIdx++;
    }

    if (nextIdx < stageOrder.length) {
      const nextStageKey = stageOrder[nextIdx];
      if (nextStageKey === "end" && !endInitialized) {
        setStagesData((prev) => {
          const baseZoom = prev.start.zoom || 1;
  ...
  const prevStage = () => {
    ...
    // Find previous used stage
    let prevIdx = currentIdx - 1;
    while (prevIdx >= 0 && !usedStages.has(stageOrder[prevIdx])) {
      prevIdx--;
    }
  ...
  /* -----------------------------
     Render
  ------------------------------*/
  // Calculate scaled dimensions for the report
  const scaledWidth = resolution ? Math.round(resolution.width * SCALE_FACTOR) : null;
  const scaledHeight = resolution ? Math.round(resolution.height * SCALE_FACTOR) : null;

  return (
  <div className="fixed inset-0 bg-black/80 z-50 flex flex-col items-center justify-center p-0 text-white" style={{overflow: 'auto'}}>
      {/* Scale report at the top */}
      <div className="mb-2 text-center text-xs text-neutral-300 font-mono">
        {resolution ? (
          <>
            <span className="font-semibold">Preview: 100% Actual Size</span>
            {" — "}
            <span>
              {scaledWidth} × {scaledHeight} px
              {" "}
              <span className="text-neutral-500">(from {resolution.width} × {resolution.height})</span>
            </span>
          </>
        ) : (
          <span className="font-semibold">Preview: 100% Actual Size</span>
  ...
  <div
    className="bg-neutral-900 rounded-xl shadow-xl border border-neutral-700"
    style={{
      width: resolution ? resolution.width + 48 : 'auto',
      minHeight: resolution ? resolution.height + 160 : 'auto',
      maxWidth: '98vw',
      maxHeight: '98vh',
      margin: 0,
      display: 'flex',
  ...
  >
        ...
        <div
          ...
          style={{
            ...
            width: resolution ? `${resolution.width}px` : "80vw",
            height: resolution ? `${resolution.height}px` : "45vh",
            overflow: "hidden",
            cursor: "grab",
        ...
        >
          ...
          {isIntro && (
            <div
              className="absolute flex flex-col items-center text-center gap-8"
              style={{
                left: "50%",
                top: "50%",
                transform,
            ...
            >
              {(() => {
                // Get intro metadata to determine which pieces are on current stage
                const introMeta = slide?.introMeta || {};
                const showIntroText = introMeta.showIntroText ?? true;
                const showShowTitle = introMeta.showShowTitle ?? true;
                const showTagline = introMeta.showTagline ?? true;
                const introTextStage = introMeta.introTextStage ?? 1;
                const showTitleStage = introMeta.showTitleStage ?? 2;
                const taglineStage = introMeta.taglineStage ?? 3;
                const introPieceOrder = introMeta.introPieceOrder || ["introText", "showTitle", "tagline"];

                // Convert stage name to number (start=1, mid=2, end=3)
                const stageNum = stage === "start" ? 1 : stage === "mid" ? 2 : 3;

                // Define piece renderers indexed by key
                const pieceRenderers = {
                  introText: {
                    shouldShow: showIntroText && introTextStage === stageNum,
                    render: () => (
                        ...
                        <h2 className="text-xl sm:text-2xl font-light tracking-widest opacity-80">
                          {(() => {
                            ...
                            if (idx !== -1) {
                              ...
                              if (resolution && resolution.width / resolution.height > 1.3) {
                  ...
                  showTitle: {
                    shouldShow: showShowTitle && showTitleStage === stageNum,
                    render: () => (
                      <h1 key="title" className="text-4xl sm:text-5xl font-bold">
                        {slide.title || "Western Living History"}
                      </h1>
                    ),
                  },
                  tagline: {
                    shouldShow: showTagline && taglineStage === stageNum && introWordGroups.length > 0,
                    render: () => (
                      <AnimatePresence key="tagline" mode="wait">
                        <motion.div
                              ...
                              return (
                                ...
                                >
                                  {group.letters.map((letter) => (
                                    <span key={letter.key} style={{ display: "inline-block", marginRight: "0.01em" }}>
                                      {letter.displayChar}
                                    </span>
                                  ))}
                                </span>
                ...
                // Build pieces array in the order specified by introPieceOrder
                const pieces = introPieceOrder
                  .filter(pieceKey => pieceKey in pieceRenderers && pieceRenderers[pieceKey].shouldShow)
                  .map(pieceKey => pieceRenderers[pieceKey].render());
          ...
          <div className="flex items-center gap-3 text-xs font-semibold whitespace-nowrap">
            ...
            {resolution && (
              <span className="text-neutral-400 font-normal">
                | Preview: {Math.round(SCALE_FACTOR * 100)}% Actual Size →{" "}
                {Math.round(resolution.width * SCALE_FACTOR)}×
                {Math.round(resolution.height * SCALE_FACTOR)} px
                <span className="text-neutral-600"> (from {resolution.width}×{resolution.height})</span>
              </span>
            )}
          </div>
          ...
          {(isIntro || isOutro) && (
            <div className="flex flex-wrap items-center gap-3 text-[11px] text-neutral-300">
              ...
              {[
                ...
                const isUsed = usedStages.has(key);
                return (
                  <label 
                    key={key} 
                    className={`flex items-center gap-1 ${!isUsed ? 'opacity-40 cursor-not-allowed' : ''}`}
                    title={!isUsed ? `${label} stage not used in this intro` : ''}
                  >
                    <span className={`text-neutral-400 ${!isUsed ? 'text-neutral-600' : ''}`}>
                      {label}
                    </span>
                    <input
                      type="number"
                      ...
                      onChange={(e) => {
                        if (!isUsed) return; // Don't allow changes to unused stages
                      ...
                      disabled={!isUsed}
                      className={`w-14 rounded border px-1 py-0.5 text-xs focus:outline-none ${
                        isUsed 
                          ? 'border-neutral-600 bg-neutral-900 text-white focus:border-blue-500' 
                          : 'border-neutral-700 bg-neutral-950 text-neutral-600 cursor-not-allowed'
                      }`}
                    />
                  </label>
                );
