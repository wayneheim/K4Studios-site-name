...
>
        ...
        <div
          ...
          style={{
            ...
            outline:
              matColor === 'none' || matColor === 'wood' ? 'none' : '1px solid #ccc',
            position: 'relative',
...
<div className="w-full flex justify-end pr-6"></div>
          <div
        ...
        <div className="flex items-center gap-3 flex-wrap justify-center mt-4">
          ...
          {['none', 'wood'].map((kind) => (
            <button
              ...
              title={kind === 'none' ? 'Print only' : 'Wood border'}
              className={`w-4 h-4 border border-gray-400 ${
                kind === 'none' ? 'bg-white' : ''
              }`}
              style={{
                borderRadius: kind === 'wood' ? '0.15rem' : 0,
                backgroundColor: kind === 'wood' ? '#deb887' : undefined,
              }}
            />
          ))}
  ...
  document.body.classList.remove("zoom-open");
        ...
        {/* 3 ▸ Caption under swatches */}
        <p className="text-xs text-gray-600 text-center mb-16 mt-2">
          {matColor === 'wood'
