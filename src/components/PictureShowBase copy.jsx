...
import { SquareChevronLeft, SquareChevronRight, Notebook, MonitorPlay, Volume2 } from "lucide-react";
...
export default function PictureShowBase({ rawData = [], basePath = "", titleBase = "" }) {
  ...
  const speakText = () => {
    // Always check the actual speech synthesis state first
    const isCurrentlySpeaking = window.speechSynthesis.speaking;
    const isPending = window.speechSynthesis.pending;

    // If any speech is active, stop it immediately
    if (isCurrentlySpeaking || isPending) {
      console.log('Stopping TTS - active speech detected');

      // Aggressive speech cancellation
      window.speechSynthesis.cancel();

      // Also try to pause as a fallback
    ...
    // Additional check: if our state thinks we're speaking but speech API doesn't,
    // reset the state (handles edge cases)
    if (isSpeaking && !isCurrentlySpeaking && !isPending) {
      console.log('Resetting stale speaking state');
      setIsSpeaking(false);
      return;
    }
    ...
    setIsSpeaking(true); // Set immediately for UI feedback
            ...
            {isEndOfStory ? (
              ...
              >
                ...
                <img
                  ...
                  className={`rounded-lg max-h-[70vh] object-contain cursor-zoom-in ${currentIndex === 0 ? '' : 'shadow-md border border-gray-300'}`}
                ...
                >
                  <h1
                    className="text-[#85644b] text-xl font-semibold mb-2"
                    style={{ opacity: 0.7, fontFamily: "'Glegoo', serif" }}
                  >
                    {currentImage?.title || titleBase || "Untitled"}
                  </h1>
                  {currentImage?.story && (
