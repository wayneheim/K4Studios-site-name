        ...
        {audioPopup && showMeta.globalAudioSrc && (
          <div style={{ position: "absolute", top: 54, left: 40, zIndex: 2000, background: "#fff", border: "1px solid #bcd", borderRadius: 10, boxShadow: "0 4px 16px rgba(0,0,0,.18)", padding: "18px 22px", minWidth: 260 }}>
            ...
            <div style={{ marginBottom: 10 }}>
              <button
                style={{ width: 90, height: 32, padding: "4px 8px", fontSize: "0.95rem", borderRadius: 5, border: showMeta.globalAudioMode === "score" ? "2px solid #19c37d" : "1px solid #aaa", background: showMeta.globalAudioMode === "score" ? "#eafaf3" : "#f6f6f6", fontWeight: 500, marginRight: 6 }}
                onClick={() => { setShowMeta(m => ({ ...m, globalAudioMode: "score" })); setAudioPopup(false); }}
              >Score</button>
              <button
                style={{ width: 90, height: 32, padding: "4px 8px", fontSize: "0.95rem", borderRadius: 5, border: showMeta.globalAudioMode === "ambient" ? "2px solid #4a90e2" : "1px solid #aaa", background: showMeta.globalAudioMode === "ambient" ? "#eaf6ff" : "#f6f6f6", fontWeight: 500 }}
                onClick={() => { setShowMeta(m => ({ ...m, globalAudioMode: "ambient" })); setAudioPopup(false); }}
              >Ambient</button>
            </div>
            <div style={{ marginBottom: 10 }}>
              <button
                style={{ width: 90, height: 32, padding: "4px 8px", fontSize: "0.95rem", borderRadius: 5, border: "1px solid #aaa", background: audioMuted ? "#eee" : "#f6f6f6", fontWeight: 500, marginRight: 6 }}
                onClick={() => { setAudioMuted(m => !m); setAudioPopup(false); }}
              >{audioMuted ? "Unmute" : "Mute"}</button>
              <button
                style={{ width: 90, height: 32, padding: "4px 8px", fontSize: "0.95rem", borderRadius: 5, border: "1px solid #c00", background: "#fbeaea", fontWeight: 500, color: "#c00" }}
