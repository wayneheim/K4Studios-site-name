// data/Galleries/Painterly-Fine-Art-Photography/Transportation/TransportationTrainsBWEntranceData.ts

export const entranceData = {
  title: "Black & White Train Photography by Wayne Heim – Painterly Fine Art Railroad Photos",
  subtitle: "Timeless Steam Engines and Locomotives in Painterly Black & White Fine Art Photography",
  description:
    "Step back in time with Wayne Heim’s painterly black and white train photography. This fine art gallery celebrates the power, nostalgia, and classic beauty of steam engines, vintage locomotives, and historic railroads—all rendered in rich monochrome. Each image is crafted to evoke the mood, drama, and elegance of railroad history, blending photographic detail with artistic, painterly technique. Experience the stories and atmosphere of the rails, where every photograph captures the enduring spirit of trains and the artistry of motion in timeless black and white.",
  details:
    "Browse painterly black and white train photography prints featuring classic steam locomotives, vintage railroad scenes, and monochrome locomotive wall art by award-winning photographer Wayne Heim. This gallery is curated for collectors, railroad enthusiasts, and lovers of fine art train photography. Key features: black and white train photography, painterly steam engine art, monochrome railroad prints, vintage locomotive photos, historic train wall art, classic railroad photography, fine art black & white train prints, American railroads, steam engine wall décor, iconic locomotives, railroad fine art, nostalgic train photography, painterly black and white railway art. Ideal for adding timeless railroad art to home, office, or gallery walls.",
  image: {
    src: "https://photos.smugmug.com/Other/Photo-Shoots/Pennsylvania/East-Broad-Top-RR-Rockhill-PA/i-Lj6wfwt/0/NNh7bcp6cfXw9ctTJTpwfqBbpB6BL7S2qVKXhq2pv/XL/_DSF0597-XL.jpg",
    alt: "Black and White Painterly Train Photography preview",
    caption: "\"Painterly Steam Engine in Black & White\"",
  },
  breadcrumb: "Transportation | Trains | Black & White",
};
