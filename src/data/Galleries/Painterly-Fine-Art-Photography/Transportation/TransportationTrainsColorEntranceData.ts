// data/Galleries/Painterly-Fine-Art-Photography/Transportation/TransportationTrainsColorEntranceData.ts

export const entranceData = {
  title: "Color Train Photography by Wayne Heim – Painterly Fine Art Railroad Photos",
  subtitle: "Colorful Locomotives, Railroads, and Trains in Painterly Fine Art Photography",
  description:
    "Experience the energy and vibrance of color train photography as you journey through Wayne Heim’s painterly fine art gallery. Explore a collection that celebrates classic locomotives, vibrant railcars, and the timeless spirit of trains—all brought to life with a painterly style that blends realism and artistic storytelling. Every image captures the strength, history, and beauty of these powerful machines, inviting you to appreciate their role in shaping our journeys and memories. These are more than photographs; they’re visual stories, evoking emotion and admiration for the golden age and present-day wonder of rail travel. Whether you’re captivated by steam engines, diesel giants, or modern trains, this gallery is a tribute to the artistry of motion in living color.",
  details:
    "Discover painterly color train photography prints featuring classic steam locomotives, modern railroads, and vibrant engines by award-winning photographer Wayne Heim. This gallery highlights fine art color trains, railroad wall art, and vivid locomotive images, blending nostalgia with painterly photo techniques. Key features: Color train photography prints, painterly railroad wall art, steam engine color photos, classic locomotives, vibrant railcars, modern trains, American railroads, fine art railway photography, collector-worthy railroad decor, historic trains, golden age of rail travel, iconic locomotives. Perfect for collectors and train enthusiasts seeking unique art prints for home, office, or gallery display.",
  image: {
    src: "https://photos.smugmug.com/Other/Photo-Shoots/Pennsylvania/Easst-Broad-Top-Railroad-Fall-23/i-VzhZFL7/0/LLKcpS25Mr83FxsP2zCzM5XqFVWZC9mbmcHG8wwHh/XL/_HF26382-Edit-XL.jpg",
    alt: "Colorful Train Photography preview",
    caption: "\"Painterly Color Locomotive\"",
  },
  breadcrumb: "Transportation | Trains | Color",
};
