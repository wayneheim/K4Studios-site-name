export const transportationFAQ = [
     // Traditional Landscape Photography FAQ – By Theme
  {
    q: "What defines Wayne's traditional landscape photography?",
    a: [
      "Wayne Heim’s traditional landscape photography focuses on authenticity and clarity. Each image captures mountains, rivers, sunsets, and waterfalls as they are—without filters or composites—celebrating detail, composition, and light in its purest form."
    ]
  },
  {
    q: "Are these real locations or digitally altered scenes?",
    a: [
      "Every photograph is rooted in a real location. From the peaks of the Canadian Rockies to the rivers of the Midwest and the waterfalls of Iceland, Wayne captures authentic landscapes in-camera. No composites, no digital trickery—just fine art prints that preserve the truth of place."
    ]
  },
  {
    q: "How are the landscape themes organized?",
    a: [
      "You can explore traditional landscapes by visual themes: <a href=\"/Galleries/Fine-Art-Photography/Landscapes/By-Theme/Mountains\">Mountains</a>, <a href=\"/Galleries/Fine-Art-Photography/Landscapes/By-Theme/Water\">Water & Waterfalls</a>, and <a href=\"/Galleries/Fine-Art-Photography/Landscapes/By-Theme/Sunsets\">Sunsets</a>. Each theme highlights a specific natural element, from grand alpine ranges to tranquil lakes and dramatic skies."
    ]
  },
  {
    q: "What makes your mountain photography unique?",
    a: [
      "Wayne’s mountain photography emphasizes both scale and fine detail. From rugged cliffs to mist-covered valleys, each image captures the timeless grandeur of the peaks while preserving a sense of intimacy and connection with the land."
    ]
  },
  {
    q: "What kinds of water scenes are included?",
    a: [
      "The collection includes everything from cascading waterfalls in Iceland to reflective lakes across the Midwest. Wayne’s traditional water photography reveals water’s dual character—dynamic power and serene calm—making these prints versatile additions to fine art collections."
    ]
  },
  {
    q: "Where were these photographs taken?",
    a: [
      "Wayne’s themed landscape galleries span both America and abroad. Locations include Wyoming, Montana, the Midwest, Iceland, Newfoundland, the Faroe Islands, and beyond—each chosen for its striking natural beauty and enduring character."
    ]
  },
  {
    q: "Are these available as prints?",
    a: [
      "Yes. All themed traditional landscapes are available as fine art prints. Formats include archival paper, vivid acrylic, and bold metal—each designed to preserve detail and last for generations. For unique texture-driven artwork, explore Wayne’s <a href=\"/Other/K4-Select-Series/Engrained\">Engrained wood series</a>."
    ]
  },
  {
    q: "Do traditional landscape prints work well for home or office decor?",
    a: [
      "Absolutely. These fine art prints bring the outdoors inside, adding atmosphere and timeless character to any space. Whether displayed in a modern office, rustic cabin, or contemporary home, Wayne’s traditional landscape photography becomes both wall art and a focal point for conversation."
    ]
  },
  {
  q: "What are 'Collector Notes' and how do I find/use them?",
  a: [
    `Ever wanted to know the deeper story behind an image, or get the artist’s creative perspective? <strong>Collector Notes</strong> are exclusive insights on select images—offering background on artistic choices, historical details, and what makes each piece extraordinary.`,
    `To read a Collector Note, just click the <strong>notepad</strong> icon at the upper right of an image (desktop), or tap the same icon in the toolbar below the image (mobile). You'll discover creative context, art history comparisons, and insider observations that help you see your print the way an art collector—or even the artist—does.`,
    `Our collection of Collector Notes is always growing, so check back for new insights and stories as you explore the gallery!`
  ]
},
];