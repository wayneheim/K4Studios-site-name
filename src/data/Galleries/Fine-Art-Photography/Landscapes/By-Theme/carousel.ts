// carousel.ts for Landscapes → By-Theme

// Import all gallery mjs modules for By-Theme from data directory
const modules = import.meta.glob('@/data/Galleries/Fine-Art-Photography/Landscapes/By-Theme/*/*.mjs', { eager: true });
...
for (const filePath in modules) {
  ...
  // Extract theme folder and filename from file path (fix group for filename)
  const match = filePath.match(/By-Theme[\\/]([^\\\/]+)[\\/]([^\\\/]+)\.mjs$/);
  ...
  const urlPath = theme === filename
    ? `/Galleries/Fine-Art-Photography/Landscapes/By-Theme/${theme}`
    : `/Galleries/Fine-Art-Photography/Landscapes/By-Theme/${theme}/${filename}`;
