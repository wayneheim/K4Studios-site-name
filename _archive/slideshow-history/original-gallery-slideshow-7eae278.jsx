...
export default function StoryShow({ images, startImageId, onExit }) {
  ...
  const [isMobileShort, setIsMobileShort] = useState(false); // NEW: phone/phablet (short side Γëñ 900)
  ...
  // Γ₧è Inactivity timer for controls
  ...
  // Γ₧ï Orientation + pointer detection
  useEffect(() => {
    const updateFlags = () => {
...
{current.story && !isMobileShort && (
  <motion.div
    ...
    {...fade}
    style={{
      // Γ£à Cap width to 520px or 92vw (whichever is smaller)
      maxWidth: "min(92vw, 520px)",
      // Γ£à Prevent the box from stretching wider than the cap
      width: "auto",
          ...
          {showControls && !isIntro && (
            ...
            >
              ...
              >
                ...
                {/* ≡ƒöè Mute/Unmute button only if audio present */}
                {hasAudio && (
                  <button
