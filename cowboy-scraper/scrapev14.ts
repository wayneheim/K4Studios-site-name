...
async function scrapeDetails(page: puppeteer.Page, sortOrder: number): Promise<ImageRecord> {
  ...
  const data = await page.evaluate(
    (sortOrderIn: number, smartSplitSrc: string) => {
      ...
      let alt =
        (document.querySelector("img.sm-lightbox-v2-photo")?.alt as string) ||
        title || "";
...
async function main() {
  ...
  try {
    ...
    while (true) {
      ...
      // Always set XL viewport size BEFORE moving to the next image -- CHANGED
      await page.setViewport({ width: 2200, height: 1800 });
      await wait(120);
      ...
      // Re-open info/details if needed
