...
async function scrapeDetails(page, sortOrder) {
  return await page.evaluate((sortOrder, smartSplit) => {
    ...
    // Image src logic: always use visible SmugMug photo, and XL version!
    let src = "";
    let alt = "";
    const photoEl = Array.from(document.querySelectorAll('img.sm-lightbox-v2-photo'))
      .find(el => el.offsetParent !== null && window.getComputedStyle(el).display !== "none");
    if (photoEl && photoEl.style.backgroundImage && photoEl.style.backgroundImage.includes('photos.smugmug.com/')) {
      const match = photoEl.style.backgroundImage.match(/url\(["']?(.+?)["']?\)/);
      if (match && match[1]) {
        src = match[1]
          .replace(/\/Ti\//g, "/XL/")
          .replace(/-Ti\.jpg$/g, "-XL.jpg")
          .replace(/\.Ti\.jpg$/g, ".XL.jpg");
        alt = photoEl.alt || title;
      }
    }
    // Fallback (old logic + double Ti swap if SmugMug CDN)
    if (!src) {
      let img =
      ...
      src = img?.src || "";
      if (src && src.includes("photos.smugmug.com/")) {
        src = src
          .replace(/\/Ti\//g, "/XL/")
          .replace(/-Ti\.jpg$/g, "-XL.jpg")
          .replace(/\.Ti\.jpg$/g, ".XL.jpg");
