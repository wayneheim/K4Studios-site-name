...
async function scrapeDetails(page: puppeteer.Page, sortOrder: number): Promise<ImageRecord> {
  ...
  for (const bp of SIZE_BREAKPOINTS) {
    ...
    // Find the <link rel="preload" as="image"> whose href contains the current id and ends with the right size
    const url = await page.evaluate(
      (currentId: string, sizeName: string) => {
        // Map sizeName to suffix
        const sizeSuffixMap: Record<string, string> = {
          srcXL: '-XL.jpg',
          srcL: '-L.jpg',
          srcM: '-M.jpg',
          srcS: '-S.jpg',
        };
        const sizeSuffix = sizeSuffixMap[String(sizeName)] || '.jpg';
        const links = Array.from(document.querySelectorAll('link[rel="preload"][as="image"]'));
        for (const link of links) {
          const href = link.getAttribute('href') || '';
          if (href.includes(currentId) && href.endsWith(sizeSuffix)) {
            return href;
          }
        }
        return '';
...
async function main() {
  ...
  try {
    ...
    // Only scrape the current image
    const data = await scrapeDetails(page, 0);
    if (!data.id) {
    ...
    results.push(data);
    console.log(
      `[1] ${data.id} | ${data.title.slice(0, 64)} | galleries: ${data.galleries.length}`
    );
  } catch (err: any) {
