...
function renderTable(title, rows, { linkPaths = false } = {}) {
  const body = rows?.length
    ? rows.map((row) => {
      const label = String(row.label || '');
      const linkedLabel = linkPaths && label.startsWith('/')
        ? `<a class="page-link" href="https://www.k4studios.com${escapeHtml(label)}" target="_blank" rel="noopener noreferrer">${escapeHtml(label)}</a>`
        : escapeHtml(label);
      return `<tr><td title="${escapeHtml(label)}">${linkedLabel}</td><td>${formatNumber(row.count)}</td></tr>`;
    }).join('')
...
export function renderDashboardV3({ summary }) {
  ...
  <div class="cards">${renderCards(summary?.counts || {})}</div><div class="grid">${renderTable('Top 25 entry pages', summary?.entryPages, { linkPaths: true })}${renderTable('Top 10 site pages', summary?.topPages, { linkPaths: true })}${renderImages(`Images accessed ${window.label.toLowerCase()}`, summary?.topImages)}${renderTable('Visitor geography', summary?.visitorGeography)}${renderImageGeography(summary?.imageGeography)}${renderTable('Entry sources', summary?.acquisition)}${renderCoreActions(summary?.actionGroups, summary?.actions)}</div>
